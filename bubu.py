...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.bubu.playlist")
pluginrootpath = "plugin://plugin.video.bubu.playlist"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.bubu.playlist/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.bubu.playlist(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.bubu.playlist" in item["path"]:
